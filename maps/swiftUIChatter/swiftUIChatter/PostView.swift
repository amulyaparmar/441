...
struct PostView: View {
    ...
    func SubmitButton() -> some View {
        ...
        Button {
            // New action closure with location-based functionality
            isDisabled = true
            var geodata = GeoData(
                lat: LocManager.shared.location.coordinate.latitude,
                lon: LocManager.shared.location.coordinate.longitude,
                facing: LocManager.shared.compassHeading,
                speed: LocManager.shared.speed
            )
            Task {
                await geodata.setPlace()
                ChattStore.shared.postChatt(
                    Chatt(username: username, message: message, geodata: geodata)
                ) {
